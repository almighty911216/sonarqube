...
public class PopulateQProfiles  extends DataChange {
  ...
  protected void execute(Context context) throws SQLException {
    ...
    massUpdate.select("select p.kee, p.organization_uuid, p.kee, p.parent_kee from rules_profiles p ");
    ...
    massUpdate.execute((row, update) -> {
      if (existQProfile(context, row.getString(1), row.getString(2))) {
        return false;
      }
  ...
  private boolean existQProfile(Context context, String uuid, String organizationUuid) throws SQLException {
    return context.prepareSelect(
      format("select count(*) from qprofiles where uuid='%s' and organization_uuid='%s'", uuid, organizationUuid)
    ).get(row -> row.getLong(1) != 0).booleanValue();
  }
}
