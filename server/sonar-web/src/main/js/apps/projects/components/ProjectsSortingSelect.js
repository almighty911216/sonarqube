...
import { SORTING_METRICS, SORTING_LEAK_METRICS } from '../utils';
...
export default class ProjectsSortingSelect extends React.PureComponent {
  ...
  constructor(props: Props) {
    super(props);
    this.state = this.parseSort(props.selectedSort);
  }

  componentDidUpdate(prevProps: Props) {
    if (prevProps.selectedSort !== this.props.selectedSort) {
      this.setState(this.parseSort(this.props.selectedSort));
    }
  }

  getOptions = () => {
    const sortMetrics = this.props.view === 'leak' ? SORTING_LEAK_METRICS : SORTING_METRICS;
    return sortMetrics.map(opt => ({
      value: opt,
      label: translate('projects.sorting', opt)
    }));
  };
  ...
  handleDescToggle = (evt: Event & { target: HTMLElement }) => {
    evt.preventDefault();
    evt.target.blur();
      ...
      <div className={this.props.className}>
        ...
        <Tooltip
          ...
          <a className="spacer-left button-icon" href="#" onClick={this.handleDescToggle}>
            {!sortDesc && <SortAscIcon className="little-spacer-top" />}
            {sortDesc && <SortDescIcon className="little-spacer-top" />}
          </a>
        </Tooltip>
      </div>
    );
